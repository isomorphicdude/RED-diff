...
save_ori=True
overwrite=True
smoke_test=10 # number of images to test
batch_size=1
num_steps=1000
...
python   main.py   exp.overwrite=True   algo=reddiff  exp.seed=1  algo.sigma_x0=0.0   algo.awd=True    algo.deg=in2_20ff     algo.lr=0.1   exp.num_steps=$num_steps    algo.sigma_y=0.0   loader.batch_size=$batch_size    loader=imagenet256_ddrmpp    dist.num_processes_per_node=1   exp.name=debug  exp.save_ori=$save_ori  exp.save_deg=$save_deg  exp.smoke_test=$smoke_test   exp.samples_root=$samples_root      exp.save_evolution=True     algo.grad_term_weight=1.0
